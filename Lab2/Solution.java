...
public class Solution{
    ...
    { int findSolution=1;
        System.out.println("\n-Bonus-\n");
        int [] generarePermStudents=new int[problem.getListOfStudents().length+1];
        for(int i=0;i<problem.getListOfStudents().length;i++) {
            bkt(i,1,generarePermStudents);
        }
        if(hallCondition) {
            System.out.println("S-a gasit solutie folosind Hall\n");
        }
        else System.out.println("Nu se poate gasi solutie folosind Hall\n");
    }
